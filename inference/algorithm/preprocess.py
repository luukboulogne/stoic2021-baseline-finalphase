...
def preprocess(input_image: sitk.Image,
    ...
    input_image = resample(input_image, new_spacing=new_spacing, outside_val=outside_val)
    input_image = sitk.GetArrayFromImage(input_image)
    input_image = center_crop(input_image, new_shape=new_shape, outside_val=outside_val)
    input_image = clip_and_normalize(input_image, clip_min=clip_min, clip_max=clip_max)
    return input_image
